...
from ..models import BudgetBucketTarget, BudgetMonth, PlaidItem, UpAccount
from ..schemas import BudgetBucketTargetCreate, BudgetBucketTargetOut, BudgetMonthCreate
from ..services.budget_buckets import list_budget_buckets, require_budget_bucket
from ..services.invoice_tracking import auto_track_receivables
from ..services.plaid_client import is_configured as plaid_is_configured, sync_transactions as sync_plaid_transactions
from ..services.reports import BUDGET_CASH_TOLERANCE, _budget_matrix_snapshot, _month_state
from ..services.up_client import (
...
def _validate_bucket_target_constraints(
    ...
    if _month_state(month_label) == "past":
    ...
    snapshot = _budget_matrix_snapshot(session, int(month_label[:4]), overrides=overrides, clear_keys=clear_keys)
    month_meta = snapshot.get("month_meta") or {}
    failing = next(
        (
            (snapshot_month, data)
            for snapshot_month, data in sorted(month_meta.items())
            if _month_state(snapshot_month) != "past"
