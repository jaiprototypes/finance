...
from ..services.archived_invoices import (
...
from ..services.invoices import create_invoice, apply_payment, update_invoice
from ..services.invoice_tracking import (
...
def get_invoice_archive_detail(archive_id: int, session: Session = Depends(get_session)):
    ...
    auto_track_receivables(session, archived_invoice_ids=[archive_id])
    session.commit()
...
def upload_invoice_archive(
    ...
    auto_track_receivables(session, archived_invoice_ids=[record.id])
    session.commit()
...
def update_invoice_archive(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(exc))
    auto_track_receivables(session, archived_invoice_ids=[archive.id])
    session.commit()
    session.refresh(archive)
    return archived_invoice_detail(session, archive)
...
def invoice_detail(invoice_id: int, session: Session = Depends(get_session)):
    ...
    auto_track_receivables(session, invoice_ids=[invoice_id])
    session.commit()
...
def create_invoice_route(payload: InvoiceCreate, session: Session = Depends(get_session)):
    ...
    auto_track_receivables(session, invoice_ids=[invoice.id])
    session.commit()
    session.refresh(invoice)
