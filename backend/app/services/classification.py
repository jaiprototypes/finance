...
from ..services.account_roles import is_business_account
from ..services.local_ai import LocalAIUnavailableError, call_local_json
from ..services.settings import get_classification_model, get_local_ai_enabled, get_setting
from ..services.subcategories import subcategory_name
...
def is_income_category_name(value: str | None) -> bool:
    canonical = canonicalize_category_name(value)
    return canonical in INCOME_CATEGORY_NAMES
...
def is_legacy_opening_transaction(description: str | None, notes: str | None, payee: str | None) -> bool:
    parts = [description or "", notes or "", payee or ""]
    hay = " ".join(parts).lower()
    return (
        "legacy opening balance" in hay
        or "legacy balance adjustment" in hay
        or "imported opening for legacy loan" in hay
    )
